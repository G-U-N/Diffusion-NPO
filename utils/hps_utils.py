...
class Selector():
    ...
    def __init__(self, device):
        ...
        checkpoint_path = os.path.join(root_path, 'HPS_v2.1_compressed.pt')
        if not os.path.exists(checkpoint_path):
            print('Downloading HPS_v2_compressed.pt ...')
            url = 'https://huggingface.co/spaces/xswu/HPSv2/resolve/main/HPS_v2.1_compressed.pt'
            r = requests.get(url, stream=True)
            with open(os.path.join(root_path, 'HPS_v2.1_compressed.pt'), 'wb') as HPSv2:
