...
def get_parser():
    ...
    parser.add_argument("--train_cond",action="store_true")
...
def aesthetic_hps_loss_fn(aesthetic_target=None,
    ...
    def loss_fn(im_pix_un, prompts):
        ...
        if aesthetic_target is None: # default maximization
            ...
            # using L1 to keep on same scale
            aesthetic_loss = abs(aesthetic_rewards - 10.0)
            # aesthetic_loss = - aes
        ...
        logits = image_features @ text_features.T
        scores = torch.diagonal(logits)
        hps_loss = abs(scores - 1.0)
        # hps_loss = - scores
        hps_loss = hps_loss.mean()
        hps_rewards = scores.mean()

        loss = (1.5 * aesthetic_loss + hps_loss * 5) /2  # 1.5 is a hyperparameter. Set it to 1.5 because experimentally hps_loss is 1.5 times larger than aesthetic_loss
        ...
        return loss, rewards, aesthetic_rewards, hps_rewards
...
def pick_score_loss_fn(inference_dtype=None, device=None):
    ...
    def loss_fn(im_pix_un, prompts):    # im_pix_un: b,c,h,w
        ...
        return loss.mean(), scores.mean(), scores.mean(), scores.mean()
...
def intern_vid2_score_fn(rm_ckpt_dir: str, precision="amp", n_frames=4):
    ...
    def score_fn(image_inputs: torch.Tensor, text_inputs: str):
        ...
        image_inputs = ((image_inputs / 2) + 0.5).clamp(0, 1)

        # # reproduce the pick_score preprocessing
        # image_inputs = image_inputs * 255   # b,c,h,w
        ...
        score = (video_features * text_features).sum(-1) # times -1 ?
        return  - score, score
...
def run_training(args, **kwargs):
    ...
    model_uncond  = instantiate_from_config(model_config)

    assert os.path.exists(args.ckpt_path), f"Error: checkpoint [{args.ckpt_path}] Not Found!"
    model = load_model_checkpoint(model, args.ckpt_path)
    model_uncond = load_model_checkpoint(model_uncond, args.ckpt_path)
    ...
    if args.mixed_precision != 'no':
        ...
        model_uncond.first_stage_model = model_uncond.first_stage_model.half()
        model_uncond.cond_stage_model = model_uncond.cond_stage_model.half()
    ...
    peft_model_uncond = peft.get_peft_model(model_uncond, config)

    peft_model_uncond.print_trainable_parameters()
    ...
    if args.lora_ckpt_path is not None:
        ...
        peft.set_peft_model_state_dict(peft_model_uncond, torch.load(args.lora_ckpt_path))

    peft_model.to(accelerator.device)
    # do we need to train the cond model?
    if args.train_cond:
    ...
    if args.inference_only:
        ...
        peft.set_peft_model_state_dict(peft_model, torch.load("[cond model weights]"))
        peft.set_peft_model_state_dict(peft_model_uncond, torch.load("[model weights]"))

        peft_model_uncond = accelerator.prepare(peft_model_uncond)
            ...
            for n in range(args.num_val_runs):
                ...
                with accelerator.split_between_processes(prompts_all) as val_prompt:
                    ...
                    with accelerator.autocast():            # mixed precision
                        ...
                        if isinstance(peft_model_uncond, torch.nn.parallel.DistributedDataParallel):
                            batch_samples = batch_ddim_sampling(peft_model,peft_model_uncond.module, cond, noise_shape, args.n_samples, \
                                                                args.ddim_steps, args.ddim_eta, args.unconditional_guidance_scale, None, decode_frame=args.decode_frame, **kwargs)
                        else:
                            batch_samples = batch_ddim_sampling(peft_model, peft_model_uncond, cond, noise_shape, args.n_samples, \
                                                                    args.ddim_steps, args.ddim_eta, args.unconditional_guidance_scale, None, decode_frame=args.decode_frame, **kwargs)
                    ...
                    for prompt,file_name in zip(prompts,filenames):
                        with open(os.path.join(dir_name,file_name+".txt"),"w") as f:
                            f.write(prompt)
    ...
    if args.train_cond:
        trainable_params = list(peft_model.parameters()) + list(peft_model_uncond.parameters())
    else:
        trainable_params = list(peft_model_uncond.parameters())
    ...
    if args.train_cond:
        peft_model, peft_model_uncond, optimizer, loss_fn = accelerator.prepare(peft_model, peft_model_uncond, optimizer, loss_fn)
    else:
        peft_model_uncond, optimizer, loss_fn = accelerator.prepare(peft_model_uncond, optimizer, loss_fn)
    # sample shape
    assert (args.height % 16 == 0) and (args.width % 16 == 0), "Error: image size [h,w] should be multiples of 16!"
    # latent noise shape
    h, w = args.height // 8, args.width // 8
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for idx, step in enumerate(range(default_iterations)): # default number of iterations is 100
            ...
            with accelerator.accumulate(peft_model_uncond):    # gradient accumulation
                with accelerator.autocast():            # mixed precision
                    ...
                    if isinstance(peft_model_uncond, torch.nn.parallel.DistributedDataParallel):
                        batch_samples = batch_ddim_sampling(peft_model.module if hasattr(peft_model,"module") else peft_model, peft_model_uncond.module, cond, noise_shape, args.n_samples, \
                        ...
                        batch_samples = batch_ddim_sampling(peft_model.module if hasattr(peft_model,"module") else peft_model, peft_model_uncond, cond, noise_shape, args.n_samples, \
                                                                args.ddim_steps, args.ddim_eta, args.unconditional_guidance_scale, None, backprop_mode=args.backprop_mode, decode_frame=args.decode_frame, **kwargs)
                    
                    if args.reward_fn in ["actpred", "vidclip", "temporal"]:    #"vidclip" is not implemented for VideoCrafter
                        ...
                        video_frames_ = video_frames_.to(peft_model.dtype)
                        ...
                        if args.reward_fn == "aesthetic" or args.reward_fn == "rainy" or args.reward_fn == "snowy" or args.reward_fn == "objectDetection" or args.reward_fn == "compression_score":
                            ...
                            loss, rewards, aesthetic_rewards, hps_rewards = loss_fn(video_frames_,prompts) 
                ...
                if accelerator.sync_gradients:
                    ...
                    # log_dict_step = {"step_loss": loss.detach().item(), "step_reward": rewards.detach().item(), "aesthetic_reward": aesthetic_rewards.detach().item(), "hps_reward": hps_rewards.detach().item()}
                    ...
                    if global_step % checkpointing_steps ==0:   
                        ...
                        if accelerator.is_local_main_process:
                            unwrapped_model = accelerator.unwrap_model(peft_model_uncond)
                            ...
                            if args.train_cond:
                                ...
                                # save lora model only
                                peft_state_dict = peft.get_peft_model_state_dict(unwrapped_model)
                                peft_model_path = os.path.join(output_dir, f"cond_peft_model_{global_step}.pt")
                                torch.save(peft_state_dict, peft_model_path)
