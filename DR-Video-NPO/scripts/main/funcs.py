...
def batch_ddim_sampling(model, model_bad, cond, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1.0,\
                        cfg_scale=1.0, temporal_cfg_scale=None, backprop_mode=None, decode_frame='-1', **kwargs):
    ddim_sampler = DDIMSampler(model,model_bad)
    ...
    for _ in range(n_samples):
        ...
        if backprop_mode is not None:   # it is for training now. Use one frame randomly to save memory
            ...
            if type(decode_frame) == int:
                ...
                idxs = range(0, samples.shape[2], 4) if decode_frame == 'alt' else range(samples.shape[2])
