...
def main(args):
    ...
    def preprocess_train(examples):
        ...
        for im_tup, label_0 in zip(im_tup_iterator, examples["label_0"]):
            if label_0 == 1: # here for adjusting the order of good and bad images
                im_tup = im_tup[::-1]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # not work
                # indices = torch.randperm(bsz // 2)
                # encoder_hidden_states[:bsz // 2] = encoder_hidden_states[indices] # random perturbate the first half prompt embeddings
