...
class CustomImageDataset(Dataset):
    def __init__(self, img_dir, sample_size):
        ...
        all_files = set(os.listdir(img_dir))  # 缓存目录中的所有文件
        self.img_names = [
            f
            for f in all_files
            if f.endswith((".png", ".jpg"))
            and f.replace(".jpg", ".txt").replace(".png", ".txt") in all_files
        ]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                with torch.no_grad():
                    with torch.autocast("cuda"):
                        ...
                        for i in range(6):
                            cond_teacher_output = target_unet(
                            ...
                            if False:
                                ...
                                uncond_teacher_output = target_unet(
                                    tmp_model_input.float(),
                                    tmp_timesteps,
                ...
                target = pred_x0.detach()
                                
                sampled_x = (
                    x_prev / alphas
                    + (sampled_nsr**2 - nsr ** 2).sqrt() * torch.randn_like(target)
                ) * sampled_alphas
                ...
                model_pred = (sampled_x - sampled_sigmas * model_pred) / sampled_alphas
            ...
            if accelerator.sync_gradients:
                ...
                global_step += 1
                update_ema(target_unet.parameters(), unet.parameters(), 0.999)
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.validation_steps == 0 or (global_step - 1)  == 0:
                        log_validation(
                            vae,
                            unet,
                            args,
